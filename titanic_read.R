...
age_map <- scale_map(train$under10, train$Survived)
...
train_alt <- data.frame(
    ...
    under10 = age_map[as.character(train$under10)])
...
for(i in 1:12){
    test_index <- sample(seq_along(train_alt$survived), 90, replace=FALSE)
    chunk1 <- mydata[-test_index,]
    chunk2 <- mydata[test_index,]
    knn(chunk1[,3:6], chunk2[,2:5], train_alt$survived, k=i)
}
...
test_knn <- function(x, i){
    ...
    chunk1 <- x[-test_index,]
    chunk2 <- x[test_index,]
    chunk2$prediction <- knn(chunk1[,3:6], chunk2[,3:6], chunk1$survived, k=i)
...
test_alt$prediction <- knn(train_alt[,3:6], test_alt[,2:5], train_alt$survived, k=6)
